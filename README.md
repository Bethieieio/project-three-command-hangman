...
The aim is to guess the word in 4-8 tries. If the user guesses the word before they run out of wrong tries, they win the game. If the user does not guess the word, it is game over. The user is then asked if they would like to play again.
With every failed guess more and more features of a stick figure would appear on the gallows until it is complete.
...
### Site Aims
- To provide a game that runs smoothly with no errors
...
### Page Layout
The game is laid out on a mock terminal on a webpage deployed on [Heroku](https://www.heroku.com/) and is connected through [GitHub](https://github.com)
...
If the user enters a letter that is not a difficulty, the game tells them this.
...
The game tells them this and adds the first part of the hangman's gallows, that also change colour for visual effect and to make it obvious that something has been added to it. The user has lost a try.
![image](assets/images/first-wrong-answer.png)

- The player then guesses two correct answers, 'I' and 'S'. These are added to the word and no trys are lost.
...
- The user tried entering 'S' again, but forgot that they already inputted in this letter. They player does not lose a life, but the game tells them that they have repeated the letter.
...
- The player has the option to cheat! But it costs one try each time they do this. To enter cheat as a word, a random letter is shown in the mystery word and the a try is taken. The game tells the player that they are a cheater! 
...
- The player guessed the word 'masking'. It was the correct word!! (Wahoo!) The game shows the completed word and tells the player well done in a graffiti font. The game asks if the player would like to play again. The player can enter 'Y' for yes and the game starts all over again with another random word and 'N' for No which ends the game.
...
- The validator thought my lines of Python were too long. To fix this, I just added the excess code to another line.
...
### Manual Testing
- Implementation: At the start of the game, the user selects the difficulty by entering 'E' for easy, 'M' for medium or 'H' for Hard. These three letters should be the only letters that allow the user to carry on with the game has it tells the game how many tries the user would like.
- Test: To test this, I input the 'E', 'M' and 'H' individually. I then tried to input other letters and words. I then added numbers and punctuation marks to see if it would stop the user from continuing.
...
- Implementation: Making sure the user can only enter words that have the same number of letters that they are trying to guess and singular letters. The game should not continue if the user's input contains any numbers, punctuation marks or any words that have more or less than the word being guessed.
- Test: I started the game, selected a difficulty and input words with numbers, words with punctuation marks and symbols and words that had more and less words than the guessed word.
...
- Verdict: Only adding letters and words with the correct amount of letters continued the game. So this was a success. 

-------------------------------------------------------------------------------------------------------------------------------------------

- Inplenmentation: When the user accidentally plays a word / letter that has already been guessed, they do not lose a try.
...
- Result: The game pops up telling the user that they have already input that letter or word. No extra try has been lost.
- Verdict: This was a success with no extra try and the information message telling the user what has happened appears.
...
- Test: I played a game and wrote cheat in the input and a letter appeared and a try was taken. I found a few letters using my brain power so most of the letters were filled in. I tried to cheat again and a new letter appeared and took away a try.
- Result: When the user types cheat, it always adds a new letter and always takes away a try.
- Verdict: I feel that this is a success, but it is very easy for the user to win by inputting cheat in every time. But I trust the users have morals and not take advantage of this...
...
- Test: By losing as many games as I could. Seeing if the image popped up at the right place each time.
- Result: Everytime the last try was lost, the image 'Game Over' popped up.
- Verdict: It was a success. The process after losing the final try was predictable.
...
- Results: When I finish a game, I inputted 'y' for yes and the game restarted. When I press any other letter the game ended.
...
## Techologies 
### Languages
- [Python](https://www.python.org/): Everything for this project is in Python.
...
- [Hangman Steps and Words](https://github.com/kiteco/python-youtube-code/blob/master/build-hangman-in-python/hangman.py) I used Kites repository to use their hangman steps because I know nothing at all about making a drawing out of symbols and adapted it to make it look more like a stick figure. I also used their words as I felt they had a good mixture of easy, difficult, long and short words.
- Rich my mentor. He is cool. He is patient. He's the best
